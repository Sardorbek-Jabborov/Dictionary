...
const setSelectorValue = (e) => {
  const getSelectvalue = e.target.textContent.trim();
  setFontFamily(getSelectvalue);
  localStorage.setItem("fontFamily", getSelectvalue); // Store the selected font family in localStorage
  select.innerHTML = ``;
  select.innerHTML += `
          <p class="header__select--selected body-m text-black-2d">
          ${getSelectvalue}
...
// Check if a font family is stored in localStorage and set it if it exists
